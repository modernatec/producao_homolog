			...
			<li>
				<a class="load" href="<?=URL::base().'admin/objects/view/'.$objeto->id?>" rel="load-content" data-panel="#direita" title="+ informações">
					...
					<div class="clear">
						...
						<span class="<?=$class_obj?> round list_faixa"><img src="<?=$calendar?>" height="12" valign='middle'> <?=($objeto->retorno != '') ? Utils_Helper::data($objeto->retorno,'d/m/Y') : 'à definir'?></span>
					</div>
					...
					?>
						<div class="clear task_line" >
							...
							<span class="round <?=$task->status->type.'_status'.$task->status->id?> list_faixa left"><?=$task->status->status;?></span>
						</div>
					<?}?>
				</a>
			</li>
