                            ...
                            foreach ($task_tags as $task) {?> 
                                ...
                                <div style='clear:both'>
                                    ...
                                    <div class="replies replies_<?=$task->id;?>"> 
                                        <? foreach ($task->status->find_all() as $task_status) {?>
                                        <div style='clear:both'>
                                            <div class='hist'>
                                                <div class="right"><?=Utils_Helper::getUserImage($task_status->user)?></div>
                                                <div class="task_reply round"> 
                                                    <? 
                                                        $show_init = true;
                                                        
                                                        if($task_status->to != ""){
                                                            if($task_status->to != $user->id){
                                                                $show_init = false;    
                                                            }                                                                
                                                        }
                                                        

                                                        if($show_init && $task_status->status_id == '5'){
                                                        ?>
                                                            <div style="padding:5px 0;">
                                                                <a class="bar_button round startTask" href="<?=URL::base();?>admin/tasks_status/start?sid=<?=$task_status->id?>" >iniciar</a>
                                                            </div>
                                                        <?}else{?>
                                                            <span class="round left list_faixa <?=$task_status->status->type?>_status<?=$task_status->status_id?>">aguardando...</span>
                                                        <?}                                                    
                                                        ...
                                                        //if($task->reply->id != '') {?>
                                                            <div class='line_bottom'>
                                                                ...
                                                                <?//}?>
                                                                    <span class="round left list_faixa <?=$task_status->status->type?>_status<?=$task_status->status_id?>"><?=$task_status->status->status?></span>
                                                                <? if($task_status->finished != ""){?>
                                                                    <span class="round left list_faixa <?=$task_status->status->type?>_status<?=$task_status->status_id?>"><?=Utils_Helper::data($task_status->finished, 'd/m/Y')?> (<?=Utils_Helper::getday($task_status->finished)?>)</span>
                                                                    </a>
                                                                <?}elseif($task_status->status_id == '6'){?>
                                                                    <span class="round left list_faixa <?=$task_status->status->type?>_status<?=$task_status->status_id?>"><?=Utils_Helper::data($task_status->created_at, 'd/m/Y')?> (<?=Utils_Helper::getday($task_status->created_at)?>)</span>
                                                                    </a>
                                                                <?}?>
                                                            </div>
                                                            <div class="clear">
                                                            <?if(!empty($task_status->description)){ ?>
                                                                <span class="wordwrap description"><?=$task_status->description;?></span>
                                                            <?}?>
                                                            </div>
                                                            <div class="options description" >
                                                                <? if($task_status->status_id == '6' && $task_status->to == $user->id){?>
                                                                    <div class="right" style="min-height:20px; padding-top:10px">
                                                                        <a href="<?=URL::base();?>admin/tasks/endtask/<?=$task_status->id?>" class="popup bar_button round">entregar</a>
                                                                    </div>
                                                                <?}?>
